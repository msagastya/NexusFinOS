import { SplitExpenseInput, SplitExpenseResult, LedgerEntry } from './types';
...
export function buildSplitExpenseTransactions(
  ...
  const realTransaction = createBaseTransaction(
    payingAccountId,
    totalAmount,
    currency,
    timestamp,
    'DEBIT' // Money is leaving the account
  );
  ...
  for (const split of splits) {
    ...
    if (split.categoryId) {
      const userShareTx = createBaseTransaction(
        payingAccountId, // Associated with the user's real account
        split.shareAmount,
        currency,
        timestamp,
        'DEBIT'
    ...
    if (split.counterpartyAccountId) {
      const receivableTx = createBaseTransaction(
        split.counterpartyAccountId,
        split.shareAmount,
        currency,
        timestamp,
        'CREDIT' // Creates or increases a receivable asset
      );
...
export function buildSettlementTransactions(
  ...
  const bankTransaction = createBaseTransaction(
    bankAccountId,
    amount,
    currency,
    timestamp,
  ...
  const receivableTransaction = createBaseTransaction(
    receivableAccountId,
    -amount, // Reducing the asset
    currency,
    timestamp,
    'DEBIT'
  );
